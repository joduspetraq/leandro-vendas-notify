...
// Filtro: campanha que NAO contem "TORRES" (as TORRES sao de outro operador)
// + type Purchase + payout > 0 (so venda de verdade).
// Estado (ids ja notificados) fica em state/seen.json, commitado pelo workflow.
import fs from "fs";
...
while (all.length < total && page <= 5) {
  const r = await fetch(
    ...
    { signal: AbortSignal.timeout(30000) } // evita job pendurado quando o RT congela
  );
