...
// Filtro: campanha que NAO contem "TORRES" + type Purchase + payout > 0
// Estado (dedupe) no TURSO (db ds24-webhook, tabela leandro_seen) — o esquema antigo de
...
await db.execute("CREATE TABLE IF NOT EXISTS leandro_seen (id TEXT PRIMARY KEY, ts INTEGER)");
await db.execute("CREATE TABLE IF NOT EXISTS leandro_kv (k TEXT PRIMARY KEY, v TEXT)");
const kvSet = (k, v) => db.execute({ sql: "INSERT INTO leandro_kv (k,v) VALUES (?,?) ON CONFLICT(k) DO UPDATE SET v=excluded.v", args: [k, String(v)] });
const kvGet = async (k) => (await db.execute({ sql: "SELECT v FROM leandro_kv WHERE k=?", args: [k] })).rows[0]?.v;
...
const vendas = all.filter((c) => !/torres/i.test(c.campaign || "") && (c.type || "") === "Purchase" && Number(c.payout) > 0);

// primeira execucao com tabela vazia: marca tudo como visto SEM notificar (migracao do seen.json)
const count = Number((await db.execute("SELECT COUNT(*) n FROM leandro_seen")).rows[0].n);
if (count === 0 && vendas.length) {
  for (const c of vendas) await db.execute({ sql: "INSERT OR IGNORE INTO leandro_seen (id, ts) VALUES (?,?)", args: [c.id, Date.now()] });
...
for (const c of vendas) {
  const seen = (await db.execute({ sql: "SELECT 1 FROM leandro_seen WHERE id=?", args: [c.id] })).rows.length;
  if (!seen) novas.push(c);
}

for (const c of novas) {
  // grava ANTES de notificar (crash nunca gera duplicada); se o ntfy falhar, desfaz pra retentar
  await db.execute({ sql: "INSERT OR IGNORE INTO leandro_seen (id, ts) VALUES (?,?)", args: [c.id, Date.now()] });
  ...
  try {
    ...
    await db.execute({ sql: "DELETE FROM leandro_seen WHERE id=?", args: [c.id] });
    console.error("ntfy falhou, vai retentar no proximo ciclo:", String(e).slice(0, 120));
  }
}

// limpa ids com mais de 3 dias
await db.execute({ sql: "DELETE FROM leandro_seen WHERE ts < ?", args: [Date.now() - 3 * 86400000] });
